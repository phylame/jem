...
import jem.util.io.defaultClassLoader
...
open class ServiceManager<T : ServiceProvider>(type: Class<T>, loader: ClassLoader? = null) {
    ...
    val services get() = serviceProviders + localRegistry.values.filterNotNull()
        ...
        try {
            while (it.hasNext()) {
                try {
                    ...
                    log.error("providers.next()", e)
                }
            }
        } catch (e: ServiceConfigurationError) {
            log.error("providers.hasNext()", e)
        }
    }
}
