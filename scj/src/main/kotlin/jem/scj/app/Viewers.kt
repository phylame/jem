...
fun viewAttributes(chapter: Chapter, names: Collection<String>, config: ViewConfig, showBracket: Boolean) {
    ...
    for (name in names) {
        when (name) {
            ...
            "text" -> values += tr("view.attrPattern", tr("view.chapterText"), chapter.text?.text ?: "")
            "names" -> {
            ...
            "size" -> {
                ...
                if (str.isNotEmpty() || !config.skipEmpty) {
                    values += tr("view.attrPattern", titleOf(name) ?: name, str)
                }
            }
        }
    }
        ...
        when (name) {
            "all" -> viewExtensions(book, book.extensions.names(), config)
                ...
                values += if (value == null) {
                    ...
                    tr("view.extPattern", name, Variants.typeOf(value), Variants.printable(value) ?: value.toString())
                }
            }
        }
    }
